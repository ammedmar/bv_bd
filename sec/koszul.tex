% !TEX root = ../bv_bd.tex

\section{Koszul duality between BV and BD}

\subsection{Koszul duality for operads}

Given an operad presented by a quadratic data \(\mathrm{O} \cong \P(E, R)\), 
its \defn{Koszul dual cooperad} is 
\[
\mathrm{O}^\ac \defeq \cC(sE; s^{2}R),
\]
and its \defn{Koszul dual cooperad} is 
\[
	\mathrm{O}^! \defeq \mathrm{S} \otimes_H \big(\mathrm{O}^{\ac}\big)^*,
\]
where \(\mathrm{S} \defeq \mathrm{End}_{\KK s^{-1}}\) is the suspension operad, see \cite[Chapter~7]{LodayVallette12} for more details. 

\begin{example}
	As explained in \cite[Section~13.3.16]{LodayVallette12}, we have
	\[
	\Pois_0^! \cong \Pois_0.
	\]
\end{example}

For operads presented by inhomogeneous quadratic data \(\mathrm{O} \cong \P(E,R)\), with \(R\subset E \oplus \cT(E)^{(2)}\), we consider the projection 
\(\q \colon \cT(E) \to \cT(E)^{(2)}\) and we define its \defn{analogue quadratic data} as \((E, \q R)\).
The \defn{quadratic analogue operad} of \(\rO\) is \(\q\mathrm{O} \defeq \P(E,\q R)\).
Under some conditions \cite[Lemma~37]{GCTV12}, satisfied by the inhomogeneous quadratic data defining the operad \(\BV\), the Koszul dual cooperad \(\q \mathrm{O}^\ac\) admits a square-zero codifferential \(d_\varphi\) which extends the map \(\varphi \colon \q R \to E\) whose graph gives \(R\). In this case, the Koszul dual cooperad is defined by
\[
\mathrm{O}^\ac \defeq \big(\q \mathrm{O}^\ac, d_\varphi\big) = \big(\cC(sE, s^2 \q R), d_\varphi\big)
\]
and the Koszul dual operad \(\mathrm{O}^!\) is defined by the same dualisation and suspension as above. 

%\subsection{Extending scalars}
%
%We will be interested in algebras over the category of graded \(\KK[[h\hbar]]\)-modules.
%Given an operad \(\rO\) over \(\gVec\), an \(\rO{[\hbar]}\)-algebra is ...

\subsection{Koszul dual Batalin--Vilkovisky algebras}

For the first time, we make fully explicit the notion of a \(\BV^!\)-algebra.

\begin{proposition}\label{prop:DualBValg}
	A \(\BV^!\)-algebra structure on \(sA\) is equivalent, under desuspension, to a Gerstenhaber \(\KK[\![\hbar]\!]\)-algebra structure 
	\((A, m , b %\{\, , \,\})
         )\)\Bruno{Shall we use \(b\) or \(\{\, , \,\}\)} on the chain \(\KK[\![\hbar]\!]\)-complex \((A,d)\), with \(|\hbar|=-2\), such that the differential \(d\) is a derivation with respect to the shifted brack \(b\) %\(\{\, , \,\}\) 
         and such that
	\begin{equation}\label{eq:dKoszuldual}
		d(m(x,y)) = m\bigl(d(x),y\bigr) + (-1)^{|x|} m\bigl(x,d(y)\bigr) + \hbar\,b(x,y), %\,\{a,b\}
	\end{equation}
for any homogeneous elements \(x, y \in A\).	
\end{proposition}

\begin{proof}
	By \cite[Proposition~7.2.1]{LodayVallette12}, the dg operad Koszul dual to the Batalin--Vilkovisky operad admits the presentation
	\[
	\BV^! \cong \bigl(\mathcal{P}(s^{-1}\mathrm{S}\otimes_H E_{\BV}^*,\, s^{-2}\mathrm{S}\otimes_H(\q R_{\BV})^{\perp}),\, (d_\varphi)^*\bigr).
	\]
	On the desuspension \(A\) of \(sA\), the linear duals \(\b^*\) and \(\m^*\) induce binary symmetric operations \(m\) and \(b\) 
	%\(\{\, , \,\}\) 
	of respective degrees \(0\) and \(1\), and the linear dual \(\Delta^*\) induces a degree \(-2\) linear operator \(\tau\).
	The relations orthogonal to the quadratic analogues of the relations \(R_{\BV}\) assert that \((A, m, b)\) %\{\, , \,\})\) 
	is a Gerstenhaber algebra with which \(\tau\) commutes, that is, a Gerstenhaber \(\KK[\![\hbar]\!]\)-algebra with \(|\hbar|=-2\).
	The induced differential \((d_\varphi)^*\) on the linear dual operad \(\bigl(\q\BV^{\ac}\bigr)^*\) is the unique derivation sending \(s^{-1}\b^*\) to \(-s^{-1}\Delta^* \circ s^{-1}\m^*\) and the other generators to \(0\).
	This produces Relation~\eqref{eq:dKoszuldual} and the fact that the differential \(d\) commutes with the action of \(\hbar\) and with the commutative product.
	The sign in Relation~\eqref{eq:dKoszuldual} comes from the fact that the differential on \(A\) is opposite to the differential on \(sA\).
\end{proof}

In plain words, the desuspension of a \(\KK\)-algebra over the operad \(\BV^!\) is a differential graded Gerstenhaber \(\KK[\![\hbar]\!]\)-algebra \((A, d, m, b %\{\, , \,\})
)\), with \(|\hbar|=-2\), except that the derivation relation with respect to the commutative product \(m\) is replaced by
\[
[d, m] = \hbar \cdot b. %\{\, , \,\}.
\]

\subsection{Algebras over cooperads}

Review of \cite{rocailucio2025absolute}.

\textcolor{blue}{To do!}

\subsection{Main theorem}

To be self-coherent, we use the homological degree convention in this definition, contrary to the abovementioned references where the Lie bracket has cohomological degree \(+1\).
Let \(\BV_{-1}\) denote the operad encoding Batalin--Vilkovisky algebras where the operator \(\Delta\) and the Lie bracket are placed in homological degree \(-1\).

\begin{theorem}
	BD-algebras are suspensions of \(\BV_{-1}^!\)-algebras, and complete BD-algebras are absolute \(\BV_{-1}^{\ac}\)-algebras.
\end{theorem}

\begin{proof}
	The first point follows directly from \cref{prop:DualBValg} with homological degrees modified accordingly.
	For the notion of absolute algebras over a cooperad, see \cite[Section~3]{rocailucio2025absolute}.
	The Koszul dual of the degree \(-1\) operator \(\Delta\) is a generating element of degree \(0\) and arity \(1\) in the Koszul dual cooperad \(\BV_{-1}^{\ac}\).
	The arguments of Example~4.6 of \emph{loc.\ cit.} show that its action on an absolute \(\BV_{-1}^{\ac}\)-algebra is equivalent to an action of the ring \(\KK[\![\hbar]\!]\).
	One concludes with the same arguments as in Section~4.2 of \emph{loc.\ cit.}.
\end{proof}

Thus BV-algebras are Koszul dual to BD-algebras, up to a mild change of degree convention.
Therefore, the operadic calculus \cite[Chapter~11]{LodayVallette12} and \cite{rocailucio2025absolute} provides bar-cobar adjunctions between categories of BV-(co)algebras and BD-(co)algebras, conilpotent or complete, which yield Quillen equivalences \cite{Vallette14}.
This offers a concrete way to relate these two algebraic structures and potentially explains duality phenomena in Quantum Field Theories.